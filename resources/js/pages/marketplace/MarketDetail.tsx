...
export default function MarketDetail() {
    ...
    const host = window.location.href.split('marketplace')[0]
    console.log(host)
    ...
    const handleSeller = (id: any, user_id:any) => {
        report({
            'user_id': user.id,
            'market_id': id,
            'description': comments,
    ...
    return (
        <div className="flex lg:max-w-[60vw] md:max-w-[100vw] sm:max-w-[100vw] max-w-[100vw] flex-col w-full gap-6 p-5">
            <MuiButton className="lg:h-[60px]  font-sans font-bold text-lg" onClick={() => navigate('/marketplace')}><FaArrowLeftLong className=" scale-150" /></MuiButton>
            <div  className="relative col-span-1 items-center">
                ...
                {market && market.user.id == user.id ? <Popover placement="bottom-end">
                    <PopoverHandler>
                ...
                </Popover> : <div className="absolute w-8 h-8 sm:top-7 top-3 text-white sm:right-7 right-5 hover:bg-gray-500 hover:bg-opacity-20 cursor-pointer rounded-md px-[13px] py-[5px]">
                            <img src="/images/home/white_dot.svg" className="w-auto h-auto" alt="" />
                        </div>}
                ...
                <div className="absolute sm:bottom-7 bottom-3 sm:left-7 left-5 flex flex-col gap-3 ">
                    ...
                    <label className="w-fit font-poppins py-2 sm:px-8 px-4 font-normal sm:text-base text-xs  text-black " style={{borderRadius: '5px'}}>{market && market.condition}</label>
                </div>
            </div>
            <div>
            ...
            <MuiButton className="rounded-b-none lg:h-[80px]  font-sans font-bold text-lg" onClick={() => handleSeller(market.id, market.user_id)}>Message Seller</MuiButton>
        </div>
    )
}
